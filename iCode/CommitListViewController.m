...
- (void) init:(UAGithubEngine*)g repoFullName:(NSString *)name
{
    ...
    //
    // get commits
    // i think get commits should be depend on branch!!??
    //
    
    [self.githubEngine commitsForRepository:self.repoFullName success:^(id response) {
        
        //NSLog(@"commit list: %@", response);
        self.commitArray = response;
        
        } failure:^(NSError * error) {
            
            NSLog(@"Error for getting commits: %@", error);
    }];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if([segue.identifier isEqualToString:@"ShowCommitDetail"])
    {
        ...
        NSString * sha = [[cell detailTextLabel] text];
        [segue.destinationViewController init:self.githubEngine repoFullName:self.repoFullName sha:sha];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Configure the cell...
    NSDictionary * commit = [self.commitArray objectAtIndex:indexPath.row];
    NSString * sha = [commit valueForKey:@"sha"];
    NSDictionary * commitDic = [commit valueForKey:@"commit"];
    NSString * message = [commitDic valueForKey:@"message"];
